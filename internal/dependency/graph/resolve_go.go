...
// isPrivateModule checks if a module path matches any configured private patterns.
// Returns true if the module should be treated as private (no proxy fetch).
func (r *GoResolver) isPrivateModule(modulePath string) bool {
	for _, pattern := range r.privatePatterns {
		// Support both exact prefix matches and glob patterns
		if strings.HasPrefix(modulePath, pattern) {
			return true
		}
		// Handle wildcard patterns like "github.com/company/*"
		if strings.Contains(pattern, "*") {
			if matched, _ := path.Match(pattern, modulePath); matched {
				return true
			}
			// Also check prefix match for patterns like "github.com/company/*"
...
// extractModulePath extracts the module path from a golang PURL.
func extractModulePath(purl string) string {
	// PURL format: pkg:golang/<path>@<version>
...
// resolvePrivateModuleEdges resolves edges for a single private module
// using vendor or git fetch.
func (r *GoResolver) resolvePrivateModuleEdges(ctx context.Context, g *Graph, rootMf *modfile.File, modulePath, version string, allMods map[string]string, edgeSet map[string]bool, files FileReader) {
	...
	queue := []workItem{{
		...
		version:    version,
		parentPURL: goModuleToPURL(modulePath, strings.TrimPrefix(version, "v")),
	}}
	...
	for len(queue) > 0 {
		...
		for _, req := range depMod.Require {
			...
			childPURL := goModuleToPURL(childPath, strings.TrimPrefix(childVersion, "v"))
			...
			if !edgeSet[edgeKey] {
				...
				if childNode != nil {
					parentNode := g.Node(item.parentPURL)
					if parentNode == nil {
						parentNode = r.findNodeByModulePath(g, item.modulePath)
					}
					if parentNode != nil {
						g.AddEdge(&Edge{
							From:  parentNode.PURL,
							To:    childNode.PURL,
							Scope: ScopeRuntime,
						})
						edgeSet[parentNode.PURL+"->"+childNode.PURL] = true
					}
				}
			}

			if _, inGraph := allMods[childPath]; inGraph && r.isPrivateModule(childPath) {
