...
func commitFile(t *testing.T, _ *git.Repository, wt *git.Worktree, repoPath, filename, content, message string) plumbing.Hash {
	t.Helper()
...
func commitFileAt(t *testing.T, _ *git.Repository, wt *git.Worktree, repoPath, filename, content, message string, when time.Time) plumbing.Hash {
	t.Helper()
	full := filepath.Join(repoPath, filename)
	if err := os.MkdirAll(filepath.Dir(full), 0o755); err != nil {
		t.Fatalf("mkdir: %v", err)
