...
func Test_Integration_CompareTags(t *testing.T) {
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            ...
            if err != nil { t.Fatalf("resolve base: %v", err) }
            targetRev, err := repo.ResolveRevision(plumbing.Revision(tt.targetTag))
            if err != nil { t.Fatalf("resolve target: %v", err) }
            basePkgs, err := ScanPackagesAtCommitSnapshot(ctx, repoDir, *baseRev)
            if err != nil { t.Fatalf("scan base: %v", err) }
            targetPkgs, err := ScanPackagesAtCommitSnapshot(ctx, repoDir, *targetRev)
            if err != nil { t.Fatalf("scan target: %v", err) }
            changes := cmp.ComparePackages(basePkgs, targetPkgs)
            if len(basePkgs)==0 && len(targetPkgs)==0 {
                if changes != nil { t.Fatalf("changes should be nil for empty inputs") }
                if tt.expectPackages { t.Fatalf("Expected packages: %s", tt.skipReason) }
                t.Skipf("No packages found: %s", tt.skipReason)
                return
            }
            if tt.baseTag == tt.targetTag && len(changes)==0 { return }
            if changes == nil { t.Fatalf("nil changes slice with non-empty inputs") }
            ...
            check := func(ps []*extractor.Package) {
                if len(ps)>0 && ps[0].Name == "" { t.Error("package missing name") }
            }
            check(basePkgs); check(targetPkgs)
        })
    }
}
