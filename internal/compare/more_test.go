...
func Test_NormalizeGopkgInURL_and_ExtractCanonical(t *testing.T) {
    ...
    for _, c := range cases {
        if got := NormalizeGopkgInURL(c.in); got != c.norm { t.Fatalf("NormalizeGopkgInURL(%q)=%q want %q", c.in, got, c.norm) }
        if got := ExtractCanonicalPackageName(c.in); got != c.canon { t.Fatalf("ExtractCanonicalPackageName(%q)=%q want %q", c.in, got, c.canon) }
    }
}
