...
// TestCollectDirectDependenciesFromCommit pins the ref-scan direct-detection
// contract: a commit snapshot must yield the same multi-ecosystem direct
// dependency coverage (Go, npm, Cargo, PyPI) as a working-tree walk, and
// vendored trees (vendor/, node_modules/) must not contribute entries.
func TestCollectDirectDependenciesFromCommit(t *testing.T) {
	root := t.TempDir()
	repo, err := git.PlainInit(root, false)
	if err != nil {
		t.Fatalf("init repo: %v", err)
	...
	files := map[string]string{
		...
		"vendor/foo/Cargo.toml":              "[dependencies]\ncargo-vendored = \"1\"\n",
	}
	...
	direct, err := CollectDirectDependenciesFromCommit(repo, hash)
	if err != nil {
	...
	tests := []struct {
		...
		{name: "go module dependency", key: "github.com/commit/dependency", want: true},
		{name: "go stdlib pseudo-dependency", key: "stdlib", want: true},
		{name: "npm dependency", key: "lodash", want: true},
