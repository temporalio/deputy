  ...
  - name: display-cased
    ...
    rules:
      - action: deny
        when: pkg.name == "x"
`,
			wantCEL:  []string{`"go","github-actions"`},
			wantSkip: []string{`"Go"`, `"GitHub Actions"`},
			wantMeta: []string{"go", "github-actions"},
		},
		{
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantMeta != nil && !slices.Equal(sources[0].Metadata.Ecosystems, tt.wantMeta) {
				t.Errorf("Metadata.Ecosystems = %v, want %v", sources[0].Metadata.Ecosystems, tt.wantMeta)
			}
		})
	}
}
