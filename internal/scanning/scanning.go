...
func ScanContainerImage(ctx context.Context, target string, targetOpts *targets.OpenOptions, opts Options) (*Execution, error) {
...
func ScanVMImage(ctx context.Context, target string, targetOpts *targets.OpenOptions, opts Options) (*Execution, error) {
...
// ScanCloudResource scans a cloud resource (AMI, EBS snapshot, etc.) for vulnerabilities.
func ScanCloudResource(ctx context.Context, target string, targetOpts *targets.OpenOptions, opts Options) (*Execution, error) {
	ctx, span := otel.StartSpan(ctx, "deputy.scanning.cloud_resource",
		trace.WithAttributes(
			attribute.String("deputy.target.path", target),
		))
	defer span.End()
	...
	// Collect inventory
	invOpts := inventory.Options{Ecosystems: opts.Ecosystems}
	invExec, err := inventory.CollectCloudResource(ctx, target, targetOpts, invOpts)
	if err != nil {
		otel.SetSpanError(span, err)
		return nil, err
	}
