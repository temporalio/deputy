...
func Test_CategorizeVulnerabilities_counts(t *testing.T) {
	...
	if stats.TotalVulns != 3 {
		t.Fatalf("TotalVulns= %d", stats.TotalVulns)
	}
	if stats.UniqueVulns != 3 {
		t.Fatalf("UniqueVulns= %d", stats.UniqueVulns)
	}
	if stats.DuplicatesFound != 1 {
		t.Fatalf("DuplicatesFound= %d", stats.DuplicatesFound)
	}
	if stats.CVECount != 1 {
		t.Fatalf("CVECount= %d", stats.CVECount)
	}
	if stats.CriticalSev != 1 {
		t.Fatalf("CriticalSev= %d", stats.CriticalSev)
	}
	if stats.HighSeverity != 1 {
		t.Fatalf("HighSeverity= %d", stats.HighSeverity)
	}
	if stats.UnknownSev == 0 {
		t.Fatalf("UnknownSev= %d", stats.UnknownSev)
	}
	if stats.FixAvailable != 1 {
		t.Fatalf("FixAvailable= %d", stats.FixAvailable)
	}
	if stats.DirectDeps != 1 {
		t.Fatalf("DirectDeps= %d", stats.DirectDeps)
	}
	if stats.IndirectDeps != 2 {
		t.Fatalf("IndirectDeps= %d", stats.IndirectDeps)
	}
}
