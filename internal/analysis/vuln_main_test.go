...
func Test_ProcessOSVVulnerability_basic_fields(t *testing.T) {
    ...
    if out.ID != "GHSA-xxxx" { t.Fatalf("unexpected ID: %v", out.ID) }
    if out.CVE != "CVE-2020-1" { t.Fatalf("expected CVE extracted, got %q", out.CVE) }
    if out.Severity == "" { t.Fatalf("expected severity set") }
    if len(out.References) == 0 || out.References[0] != "https://example.com" { t.Fatalf("unexpected refs: %v", out.References) }
    if len(out.FixedVersions) == 0 || out.FixedVersions[0] != "v1.2.3" { t.Fatalf("unexpected fixes: %v", out.FixedVersions) }
    if out.Published == "" || out.Modified == "" { t.Fatalf("expected published/modified set") }
}
...
func Test_ProcessOSVVulnerability_no_aliases_severity(t *testing.T) {
    ...
    if out.CVE != "" { t.Fatalf("unexpected CVE: %q", out.CVE) }
}
