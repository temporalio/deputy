...
func goFinding(advID, source, name, version string) *vulnerabilityv1.Finding {
	return &vulnerabilityv1.Finding{
		AdvisoryId: advID,
		Package:    goPkg(name, version),
		Sources:    []string{source},
		Affected:   true,
	}
}

func onlyPackage() []vulnerabilityv1.ArtifactKind {
	return []vulnerabilityv1.ArtifactKind{vulnerabilityv1.ArtifactKind_ARTIFACT_KIND_PACKAGE}
}
...
func TestRegistryUnionWithProvenance(t *testing.T) {
	shared := "CVE-SHARED"
	a := &fakeSource{
		name: "a", ecosystems: []string{"go"}, artifacts: onlyPackage(),
		findings: []*vulnerabilityv1.Finding{goFinding(shared, "a", "github.com/foo/bar", "1.0.0")},
	}
	b := &fakeSource{
		name: "b", ecosystems: []string{"go"}, artifacts: onlyPackage(),
		findings: []*vulnerabilityv1.Finding{
