...
func extractConfig(cfg v1.Config) ImageConfig {
	ic := ImageConfig{
		User:       cfg.User,
		Env:        cfg.Env,
...
func extractMetadata(cf *v1.ConfigFile, img v1.Image) ImageMetadata {
	meta := ImageMetadata{
...
func extractHistory(history []v1.History) []ImageHistoryEntry {
	if len(history) == 0 {
		return nil
	}
	entries := make([]ImageHistoryEntry, 0, len(history))
...
func (c *ImageConfig) IsRootUser() bool {
...
func (c *ImageConfig) HasSensitiveEnv() []string {
	return security.DetectSensitiveEnvFromList(c.Env)
}

// ToMap converts ImageInfo to a map for CEL policy evaluation.
...
func (i *ImageInfo) ToMap() map[string]any {
	if i == nil {
...
func (i *ImageInfo) configToMap() map[string]any {
	c := i.Config
	m := map[string]any{
		"user":          c.User,
		"is_root":       c.IsRootUser(),
		"env":           toAnySlice(c.Env),
		"sensitive_env": toAnySlice(c.HasSensitiveEnv()),
		"entrypoint":    toAnySlice(c.Entrypoint),
		"cmd":           toAnySlice(c.Cmd),
		"working_dir":   c.WorkingDir,
		"exposed_ports": toAnySlice(c.ExposedPorts),
		"volumes":       toAnySlice(c.Volumes),
		"labels":        toAnyMap(c.Labels),
		"shell":         toAnySlice(c.Shell),
		"stop_signal":   c.StopSignal,
		"on_build":      toAnySlice(c.OnBuild),
	}

	if c.Healthcheck != nil {
		m["healthcheck"] = map[string]any{
			"test":     toAnySlice(c.Healthcheck.Test),
...
func (i *ImageInfo) metadataToMap() map[string]any {
	m := i.Metadata
...
func (i *ImageInfo) historyToMaps() []any {
	if len(i.History) == 0 {
		return nil
	}
