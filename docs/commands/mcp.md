...
`remediableCount` includes verified in-place fixes, migration fixes, and
unverified advisory-claimed fixes that Deputy can still turn into a command.
`migrationCount` is the subset that requires moving to a different package or
module path, so commands may be non-executable manual migration guidance. These
counts are vulnerability-level counts. `commandCount`, `executableCommandCount`,
and `manualCommandCount` describe the deduplicated remediation plan in
`commands`; they can be lower than `remediableCount` when multiple
vulnerabilities share one package upgrade or migration.
For migration fixes, each command includes package identity fields so agents can
follow up with `graph_why` using the emitted `purl` or `package` instead of
parsing command text. For indirect migration commands, set
`resolveTransitives` to `true` on the follow-up `graph_why` call when you need
the direct importer path; that may use package registry, deps.dev, or Git
lookups.
