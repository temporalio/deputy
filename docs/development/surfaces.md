...
A command's machine-readable output belongs to whoever owns the format, and that, not the presence of a human reader, is what decides the category. Where the format is ours, the output *is* the proto: `deputy triage --format json` marshals the response message with protojson, which makes it a direct projection, and a hand-written struct with JSON tags beside it would be a second contract nobody agreed to, drifting from the one the API already serves. Where the format is standardized, it is a foreign contract and kind 4 applies instead: `--format sarif` builds a SARIF log through `sarif.Convert`, and SBOM output is written by Protobom. Deriving a published schema from our proto is backwards.
...
The package-manager proxies, and the standardized formats a command emits.
...
The report formats are the same relationship in a different shape. A SARIF log or an SBOM answers to a schema published elsewhere, so the mapping runs outward from our messages and never back: [`internal/sarif`](../../internal/sarif) converts a report into somebody else's document, and [`internal/sbom`](../../internal/sbom) hands the work to Protobom, which owns CycloneDX and SPDX serialization. What these owe is fidelity to the published schema, which is a conformance test, not correspondence with a proto.
...
4. For the DSL, derive the vocabulary where a canonical set exists to derive from, and edit both places where one does not. Severity names are the derived case: `policy.SeverityConstantNames()` in [`internal/policy/evaluator.go`](../../internal/policy/evaluator.go) is what the runtime binds, and `internal/policy/lsp/completions.go` calls it instead of keeping a list. Scopes are the counterexample: the evaluator owns `scopeConstants` and the LSP spells out the same members, so extending that vocabulary means editing both. Either way, add the correspondence test, because nothing today relates a completion list to the runtime it teaches. Apply the admission test before adding a helper.
