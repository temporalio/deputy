...
- **Reflection over the Go struct** for the configuration file reference, since that struct is the contract today. The `yaml` and `json` tags already name every file key, so those tables can be rendered from them the way `internal/docsgen` renders the entrypoint reference, with the same drift test holding the edge. Not the `DEPUTY_*` list: reflection can see the tags, and the tags do not encode the environment contract, so it cannot recover a variable's name, its parser, or whether the field has an environment binding at all. That list needs the binding modeled first, as an `env` tag or a registry the loader reads, and then it derives from the same mechanism. If configuration ever moves into proto, this becomes an ordinary direct projection and the mechanism above applies instead.
