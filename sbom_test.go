...
func key(system pb.System, name, version string) string { return string(system) + "|" + name + "|" + version }
...
type fakeFetcher struct{ fs billy.Filesystem; root string }
...
func Test_mapPURLToDepsDev_more(t *testing.T) {
    ...
    for _, tc := range cases {
        p, err := packageurl.FromString(tc.purl)
        if err != nil { t.Fatalf("parse purl %q: %v", tc.purl, err) }
        sys, name, ver := mapPURLToDepsDev(p)
